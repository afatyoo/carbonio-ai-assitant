...
try {
	...
	await writeFile(
		checksumPath,
		`${checksum}  carbonio-ai-assistant-v2.0.0.tar.gz\n`,
		'utf8'
	);
	...
	assert.match(output, /Tag: `v2\.0\.0`/);
	assert.match(output, new RegExp(`Commit: \`${commit}\``));
	assert.match(output, new RegExp(`Release workflow: ${runUrl}`));
	assert.match(output, new RegExp(`Artifact checksum: \`${checksum}  carbonio-ai-assistant-v2\\.0\\.0\\.tar\\.gz\``));
	...
	assert.match(repositoryReadme, /carbonio-ai-assistant-v2\.0\.0\.tar\.gz/);
	...
	assert.match(repositoryReport, /^# Carbonio AI Assistant v2\.0\.0/m);
	...
	const extendedReportPath = path.join(brokenRoot, 'docs/releases/v2.0.0.md');
