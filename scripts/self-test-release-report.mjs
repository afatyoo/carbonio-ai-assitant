...
try {
	...
	await writeFile(reportPath, '# Carbonio AI Assistant v2.3.2\n\nVerified report body.\n', 'utf8');
	await writeFile(
		checksumPath,
		`${checksum}  carbonio-ai-assistant-v2.3.2.tar.gz\n`,
		'utf8'
	);
	...
	assert.match(output, /Tag: `v2\.3\.2`/);
	assert.match(output, new RegExp(`Commit: \`${commit}\``));
	assert.match(output, new RegExp(`Release workflow: ${runUrl}`));
	assert.match(output, new RegExp(`Artifact checksum: \`${checksum}  carbonio-ai-assistant-v2\\.3\\.2\\.tar\\.gz\``));
	...
	await assert.rejects(
		...
		/Tag v2\.3\.3 does not match version 2\.3\.2/
	);
	...
	assert.match(repositoryReadme, /carbonio-ai-assistant-v2\.3\.2\.tar\.gz/);
	...
	assert.match(repositoryReport, /^# Carbonio AI Assistant v2\.3\.2/m);
	...
	assert.equal(contract.version, '2.3.2');
	...
	const extendedReportPath = path.join(brokenRoot, 'docs/releases/v2.3.2.md');
