...
try {
	...
	await writeFile(
		checksumPath,
		`${checksum}  carbonio-ai-assistant-v2.0.1-beta.1.tar.gz\n`,
		'utf8'
	);
	...
	assert.match(output, /Tag: `v2\.0\.1-beta\.1`/);
	assert.match(output, new RegExp(`Commit: \`${commit}\``));
	assert.match(output, new RegExp(`Release workflow: ${runUrl}`));
	assert.match(output, new RegExp(`Artifact checksum: \`${checksum}  carbonio-ai-assistant-v2\\.0\\.1-beta\\.1\\.tar\\.gz\``));
	...
	await assert.rejects(
		...
		/Tag v2\.0\.2 does not match version 2\.0\.1-beta\.1/
	);
	...
	assert.equal(rootPackage.version, '2.0.1-beta.1');
	...
	assert.match(repositoryReadme, /carbonio-ai-assistant-v2\.0\.1-beta\.1\.tar\.gz/);
	...
	assert.match(repositoryReport, /^# Carbonio AI Assistant v2\.0\.1-beta\.1/m);

	const contract = await validateRepositoryReleaseContract({
		projectRoot,
		expectedVersion: '2.0.1-beta.1'
	});
	assert.equal(contract.version, '2.0.1-beta.1');
	assert.equal(contract.closedBugCount, 43);
	...
	const contractFiles = [
		...
		'docs/releases/v2.0.1-beta.1.md',
		'README.md',
		'CHANGELOG.md',
	...
	const extendedReportPath = path.join(brokenRoot, 'docs/releases/v2.0.1-beta.1.md');
	...
	assert.equal(extendedContract.closedBugCount, 44);
