...
try {
	...
	await writeFile(reportPath, '# Carbonio AI Assistant v2.2.1\n\nVerified report body.\n', 'utf8');
	await writeFile(
		checksumPath,
		`${checksum}  carbonio-ai-assistant-v2.2.1.tar.gz\n`,
		'utf8'
	);
	...
	assert.match(output, /Tag: `v2\.2\.1`/);
	assert.match(output, new RegExp(`Commit: \`${commit}\``));
	assert.match(output, new RegExp(`Release workflow: ${runUrl}`));
	assert.match(output, new RegExp(`Artifact checksum: \`${checksum}  carbonio-ai-assistant-v2\\.2\\.1\\.tar\\.gz\``));
	...
	await assert.rejects(
		...
		/Tag v2\.2\.2 does not match version 2\.2\.1/
	);
	...
	assert.equal(rootPackage.version, '2.2.1');
	...
	assert.match(repositoryReadme, /carbonio-ai-assistant-v2\.2\.1\.tar\.gz/);
	...
	assert.match(repositoryReport, /^# Carbonio AI Assistant v2\.2\.1/m);

	const contract = await validateRepositoryReleaseContract({
		projectRoot,
		expectedVersion: '2.2.1'
	});
	assert.equal(contract.version, '2.2.1');
	...
	const extendedReportPath = path.join(brokenRoot, 'docs/releases/v2.2.1.md');
