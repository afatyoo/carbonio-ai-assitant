...
try {
	...
	await writeFile(reportPath, '# Carbonio AI Assistant v2.1.0\n\nVerified report body.\n', 'utf8');
	await writeFile(
		checksumPath,
		`${checksum}  carbonio-ai-assistant-v2.1.0.tar.gz\n`,
		'utf8'
	);
	...
	assert.match(output, /Tag: `v2\.1\.0`/);
	assert.match(output, new RegExp(`Commit: \`${commit}\``));
	assert.match(output, new RegExp(`Release workflow: ${runUrl}`));
	assert.match(output, new RegExp(`Artifact checksum: \`${checksum}  carbonio-ai-assistant-v2\\.1\\.0\\.tar\\.gz\``));
	...
	await assert.rejects(
		...
		/Tag v2\.1\.1 does not match version 2\.1\.0/
	);
	...
	assert.match(repositoryReadme, /carbonio-ai-assistant-v2\.1\.0\.tar\.gz/);
	...
	assert.match(repositoryReport, /^# Carbonio AI Assistant v2\.1\.0/m);
	...
	const extendedReportPath = path.join(brokenRoot, 'docs/releases/v2.1.0.md');
